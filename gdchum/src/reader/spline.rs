...
pub fn read_spline(data: &Vec<u8>, fmt: libchum::format::TotemFormat) -> Option<Dictionary> {
    let spline = match spline::Spline::read_from(&mut data.as_slice(), fmt) {
        Ok(x) => x,
        Err(_) => {
            godot_print!("SPLINE file invalid");
            return None;
        }
    };
    let mut data = Dictionary::new();
    data.set(&"unk4".into(), &spline.unk4.to_vec().to_variant());
    data.set(&"vertices".into(), &spline
        .get_vertices_as_vec()
        .into_iter()
...
pub fn read_spline_from_res(data: &ChumFile) -> Dictionary {
    ...
    match read_spline(&data.get_data_as_vec(), fmt) {
        Some(mesh) => {
