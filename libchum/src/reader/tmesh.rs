...
fn read_strip<R: Read>(file: &mut R) -> io::Result<Strip> {
    ...
    let vertex_ids: Vec<u16> = (0..num_elements).map(|_| {
...
fn read_strip_ext<R: Read>(file: &mut R) -> io::Result<StripExt> {
    let num_elements: u32 = file.read_u32::<BigEndian>()?;
    let elements: Vec<ElementData> = (0..num_elements).map(|_| {
        Ok(ElementData {
...
impl TMesh {
    pub fn gen_triangles(&self) -> Vec<Vec<Tri>> {
        self.strips.iter()
                   .zip(&self.strips_ext)
                   .map(|(strip, strip_ext)| strip_gen_triangles(strip, strip_ext, &self.vertices, &self.texcoords, &self.normals))
                   .collect()
    }
    ...
    pub fn read_from<R: Read>(file: &mut R) -> io::Result<TMesh> {
        ...
        let vertices: Vec<Vector3> = (0..num_vertices).map(|_| {
            Ok(Vector3 {
        ...
        let texcoords: Vec<Vector2> = (0..num_texcoords).map(|_| {
            Ok(Vector2 {
                x: file.read_f32::<BigEndian>()?,
                y: file.read_f32::<BigEndian>()?
            })
        ...
        let normals: Vec<Vector3> = (0..num_normals).map(|_| {
            Ok(Vector3 {
                x: file.read_f32::<BigEndian>()?,
                y: file.read_f32::<BigEndian>()?,
                z: file.read_f32::<BigEndian>()?
        ...
        let strips: Vec<Strip> = (0..num_strips).map(|_| {
            read_strip(file)
        }).collect::<io::Result<_>>()?;
        // Ignore a few bytes
        io::copy(&mut file.take((num_strips as u64) * (padding_mult as u64)), &mut io::sink())?;
        // Read stripext data
        let num_strips_ext: u32 = file.read_u32::<BigEndian>()?;
        let strips_ext: Vec<StripExt> = (0..num_strips_ext).map(|_| {
            read_strip_ext(file)
        }).collect::<io::Result<_>>()?;
