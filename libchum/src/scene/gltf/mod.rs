...
fn generate_primitive(
    ...
    let texcoord_accessor = json::Accessor {
        buffer_view: Some(points_index),
        byte_offset: mem::size_of::<Vector2>() as u32,
        count: point_buf.len() as u32,
        component_type: Valid(json::accessor::GenericComponentType(json::accessor::ComponentType::F32)),
        extensions: Default::default(),
        extras: Default::default(),
...
fn export_mesh(mesh: &scene::Mesh, buffer: &mut Vec<u8>, root: &mut json::Root) -> json::Mesh {
            ...
            for strip in strips {
                ...
                /*match strip.tri_order {
                    ...
                    TriStripOrder::CounterClockWise => {
                        // if I could put just the iterator in another match arm I would be happy
                        for corner in strip.elements.iter().rev() {
                            let point = Point {
                                vertex: mesh.vertices[corner.vertex_id as usize],
                                texcoord: mesh.texcoords[corner.texcoord_id as usize],
                                normal: mesh.normals[corner.normal_id as usize]
...
fn export_node(node: &scene::SNode, mesh_indices: &HashMap<String, u32>, root: &mut json::Root) -> json::Index<json::Node> {
    // let children = node.children.iter().map(|c| export_node(c, mesh_indices)).collect();
    ...
    let node = json::Node {
        ...
        mesh: node.visual_instance.as_ref().map(|id| {
            let index = mesh_indices.get(id).unwrap();
...
pub fn export_scene(scn: &scene::Scene) -> json::Root {
    ...
    for (_id, elem) in scn.visual_instances.iter() {
        ...
        match vis {
            scene::SVisualInstance::Mesh { mesh } => {
                let json_mesh = export_mesh(&mesh, &mut buffer, &mut root);
